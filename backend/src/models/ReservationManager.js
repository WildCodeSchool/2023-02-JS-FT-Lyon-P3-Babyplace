...
class ReservationManager extends AbstractManager {
  ...
  findReservationForProByDate(dateArray, proId) {
    return this.database.query(
      `SELECT t.id reservationId, t.status, c.firstname, c.lastname, p.id parentId FROM ${this.table} as t JOIN child AS c ON t.child_id = c.id JOIN place as pl ON t.place_id = pl.id JOIN parent as p ON c.parent_id = p.id WHERE pl.pro_id = ? AND t.reservation_date in (?) AND t.status in (0, 1)`,
      [proId, dateArray]
    );
  }
